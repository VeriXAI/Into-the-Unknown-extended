...
def run_script(classes=None, seed=0, epochs=10, epochs_ratio=0.2, model_path=None, experiment_name="EX",
    ...
    if model_path is None:
        model_path = "AT_CNY19id1_F_MNIST_{}.h5".format(classes_string)
        if plot_name == "!":
            plot_name_current = "AT_CNY19id1_F_MNIST_{}".format(classes_string)
        else:
            plot_name_current = plot_name

    raw_monitor = box_abstraction_F_MNIST()
    ...
    monitor_manager = MonitorManager([distance_monitor], clustering_threshold=clustering_threshold,
                                     skip_confidence=False, fit_distribution_method=None,
