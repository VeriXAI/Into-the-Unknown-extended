...
def run_explanation_alpha_thresholding():
    ...
    model, _ = get_model(model_name=model_name, model_path=model_path, data_train=data_train_model,
                         data_test=data_test_model, class_label_map=None, model_trainer=None, n_epochs=None,
                         batch_size=None, statistics=Statistics())

    # compute results
    probabilities = model.predict_proba(data_run.inputs())  # TODO CS: probably have to map the labels/classes
