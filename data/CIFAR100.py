...
def load_CIFAR_100(data_train_model: DataSpec, data_test_model: DataSpec, data_train_monitor: DataSpec,
    ...
    cifar100_dataset_folder_path = DATA_PATH + "cifar-100-python/cifar-100-batches-py"
    # preprocess_and_save_data(cifar100_dataset_folder_path, normalize, one_hot_encode)
    n_batches = 1
    x_train = []
    y_train = []
    ...
    if adversarial_data_suffix is not None:
        with open(cifar100_dataset_folder_path + "/adversarial{}".format(adversarial_data_suffix), mode='rb') as file:
...
def load_cfar100_batch(cifar100_dataset_folder_path, batch_id):
    with open(cifar100_dataset_folder_path + "/data_batch_" + str(batch_id), mode='rb') as file:
