...
def obtain_predictions(model, data, class_label_map, layers=None, ignore_misclassifications: bool = False,
                       transfer: bool = False, use_tf_format: bool = False):
    assert not use_tf_format or not ignore_misclassifications, "incompatible options"
    delta_t = time()
    data_filtered = data
    if layers is None:
        ...
        for layer_index in layers:
            try:
                ...
                manual_model = False
            if manual_model:
                assert not use_tf_format, "unsupported option"
                ...
                if use_tf_format:
                    result = model_until_layer.predict(data_filtered)
                else:
